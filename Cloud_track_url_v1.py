...
def wait_for_redirect_smart(page, initial_url, retailer_name):
    try:
        timeout = 50000 if any(k in retailer_name for k in ["Hotels", "JCPenney", "Lowe", "Robinhood", "Zappos", "StubHub"]) else 30000
    ...
    try:
        ...
        for i, cat in enumerate(cat_links):
            ...
            if cat["link"]:
                ...
                time.sleep(2)
                wait_for_redirect_smart(page, cat["link"], retailer)
    ...
    with sync_playwright() as p:
        ...
        for index, row in df_input.iterrows():
            ...
            if result and (str(result.get("DD Name")).lower() == "dd cannot be found" or is_data_complete(result)):
                ...
                if is_new_entry:
                    # 💡 針對清單中完全沒出現過的 Retailer，即使抓失敗也要強制初始化一筆，防止消失
